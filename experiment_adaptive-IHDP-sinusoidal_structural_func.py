...
from dr_kte_projected import xMMD2_projected_fold_generic
...
def collect_epsilon_greedy_from_X(
    X_all,
    ns,
    d,
    beta_vec,
    ...
    if ns > N_total and not sample_replace:
        raise ValueError("ns > number of rows in X_all and sample_replace=False")

    # sample indices from X_all (without replacement by default)
    idx_sample = rng.choice(np.arange(N_total), size=ns, replace=False)
    X = X_all[idx_sample].copy()  # shape (ns, d)
        ...
        for method in method_list:
            for ns in tqdm(ns_list):
                ...
                for n in range(num_experiments):
                    ...
                    elif method == "Projected-DR-KTE":
                        value = xMMD2_projected_fold_generic(
                            Y=Y,
                            w=w,
                            X=X,
                            A=T,
...
ns_list = np.array(list(np.arange(100, 908, 100)) + [908])
b_list = ["I"]
method_list = ["Projected-DR-KTE"]
# method_list = ['VS-DR-KTE', 'DR-xKTE', 'IPW-xKTE']


experiment = "adaptive_ihdp"
name_folder = "results/" + str(experiment) + "/"
run_tests_adaptive_with_X(
    X_all,
    b_list,
    method_list,
    ns_list,
...
name_folder_list_adaptive_null = ["results/" + "adaptive_ihdp" + "/"]
ns_list_false_null = ns_list
ns_array_false_null = np.array(ns_list_false_null)
b_list_false_null = ["I"]
methods_false_null = ["Projected-DR-KTE_alternating"]
...
method_dict = {
    "Projected-DR-KTE_alternating": "Projected-DR-KTE-a",
    "Projected-DR-KTE_chronological": "Projected-DR-KTE-c",
    "DR-xKTE": "AIPW-xKTE",
    "IPW-xKTE": "IPW-xKTE",
    "KTE": "KTE",
}
...
b = "I"
ns = 908
method = "Projected-DR-KTE_alternating"
...
method_list = ["Projected-DR-KTE_alternating"]
...
plt.tight_layout()
plt.savefig(
    f"plots/REVIEWED_null_dr_adaptive_IHDP{case}_errorbar.png", bbox_inches="tight"
)
plt.show()
...
# # ns_list = np.array(list(np.arange(100, 450, 50)) + [908])
# ns_list = np.array(list(np.arange(100, 908, 50))+ [908])
...
# experiment = 'adaptive_ihdp'
# name_folder = 'results/' +str(experiment) + '/'
# run_tests_adaptive_with_X(X_all, b_list, method_list, ns_list, name_folder, num_experiments, iterations, split="alternating")
...
#     name_folder="results/adaptive_ihdp/",
#     split_suffix="_alternating.csv",
#     alpha=0.05,
#     save_path="plots/adaptive_ihdp_alternative_power.png"
# ):
#     # === Styling ===
...
#     plt.rcParams["axes.grid"] = True
#     # === Data Preparation for Uniform X-Axis ===
#     # 1. Create a simple index for plotting positions (0, 1, 2, ...)
#     x_positions = np.arange(len(ns_list))
#     # 2. Labels for ticks are the string versions of the sample sizes
#     ns_labels = [str(n) for n in ns_list]
#     ns_labels = [str(n) if (n == 100) or (n == 908) else "" for n in ns_list]
...
#             ax.axvline(x_pos, color='#cccccc', linestyle=':', linewidth=0.8, zorder=0)
#         if col == 0:
...
#             ax.errorbar(
#                 x=x_positions, # Use the index for uniform spacing,
#                 y=power_values,
#                 yerr=yerr,
#                 capsize=4,
...
#         # Capture handles/labels from the first subplot for the unified legend
#         if col == 0:
...
#     split_suffix='_alternating.csv',
#     alpha=0.05,
#     save_path='plots/scenarios_II_III_IV_adaptive_ihdp_power.png'
# )
...
# b_list = ['II', 'III', 'IV']
# method_list = ['VS-DR-KTE', 'CADR', 'Hadad']
# method_names = ['VS-DR-KTE', 'CADR', 'Hadad']
# for b in b_list:
#     for method in method_list:
...
# rejection_proportion_se = np.zeros((len(b_list), len(method_list)))
# confidence_level = 0.05

# for i, b in enumerate(b_list):
#     for j, method in enumerate(method_list):
#         name = name_folder + 'ns' + str(ns) + 'b' + str(b) + method + '_alternating.csv'
#         pvals = d[name]['p_values']            # array-like of p-values for this config
#         N = len(pvals)
#         if N == 0:
#             rejection_proportion[i, j] = np.nan
#             rejection_proportion_se[i, j] = np.nan
#             continue

#         # estimated rejection probability
#         p_hat = (pvals < confidence_level).mean()
#         # Monte-Carlo standard error for a proportion (Bernoulli)
#         se = 1.96*sqrt(p_hat * (1.0 - p_hat) / N)

#         rejection_proportion[i, j] = p_hat
#         rejection_proportion_se[i, j] = se

# # show arrays
# print(rejection_proportion), print(rejection_proportion_se)
