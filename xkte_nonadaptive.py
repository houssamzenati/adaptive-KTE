...
def xMMD2dr(XY, w, Xcov, T, kernel_function, lam, **kwargs):
    ...
    mu1a = np.linalg.solve(
        KX[m1 : m1 + n1, m1 : m1 + n1] + n1 * gamma * np.eye(n1),
        KX[m1 : m1 + n1, : m1 + n1],
    )
...
def kernel_two_sample_test_agnostic(
    Y, T, w, kernel_function="rbf", p=0.5, verbose=False, random_state=None, **kwargs
):
...
def kernel_dr_two_sample_test_agnostic(
    ...
    **kwargs
    ...
    xmmd2dr = xMMD2dr(Y, w, Xcov, T, kernel_function, lam, **kwargs)
    if verbose:
