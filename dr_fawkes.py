...
def MMD2dru(XY, w, Xcov, T, idx, idx2, kernel_function, **kwargs):
    ...
    mu1a = np.linalg.solve(
        KX[m1 : m1 + n1, m1 : m1 + n1] + gamma * np.eye(n1),
        KXx[m1 : m1 + n1, mj1 + nj1 :],
    )
    ...
    mu0b = np.linalg.solve(
        ...
        KXx[m1 + n1 : m + n1, : mj1 + nj1],
    )
    zeroed_mu0b = np.vstack((mu0b, np.zeros((n - n1, mj1 + nj1))))
    mu1b = np.linalg.solve(
        KX[m + n1 :, m + n1 :] + gamma * np.eye(n - n1), KXx[m + n1 :, : mj1 + nj1]
    )
    ...
    # DR
    part1 = np.vstack(
        (zeroed_mu1a - zeroed_mu0a - wwb * muAa, wwb * np.eye(mj + nj - (mj1 + nj1)))
    )
...
def kernel_dr_nonuniform(
    ...
    **kwargs
    ...
    mmd2dru_null = compute_null_distribution(
        Y,
        Xcov,
        w,
        T,
